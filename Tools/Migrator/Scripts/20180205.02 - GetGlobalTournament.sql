/****** Object:  StoredProcedure [dbo].[GETGLOBALTOURNAMENT]    Script Date: 2/5/2018 3:33:54 PM ******/
SET ANSI_NULLS ON
GO
...
-- Updated : 
-- Adds the conditions incase the operator filter is null
-- =============================================
ALTER PROCEDURE [dbo].[GETGLOBALTOURNAMENT]
...
BEGIN
	...
	WHERE 
		a.RelationType = 6
		AND (@Operator IS NULL OR  EXISTS(SELECT 1 FROM @OperatorFilter pf WHERE pf.[Operator] =  a.RelationId))
	GROUP BY a. TournamentId
	----------------------------------------------------------------------------------------------
	/*
