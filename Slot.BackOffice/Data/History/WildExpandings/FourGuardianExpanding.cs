...
{
    public class FourGuardianExpanding : IWildExpanding
    {
        ...
        private static void WildVerticalExpanding(WheelViewModel wheel)
        {
            for (var reel = 0; reel < 5; reel++)
                if (CheckWild(reel, (int) FourGuardianSymbol.WildVertical, wheel))
                    for (var index = 0; index < 3; index++)
                    {
                        var sym = wheel.reels[reel][index].symbol;
                        if ((sym < 10) || (sym == (int) FourGuardianSymbol.WildVertical))
                            wheel.reels[reel][index] = new Symbols
                            {
                                symbol = (int) FourGuardianSymbol.WildBase
                            };
                    }
        }
        ...
        {
            var wild = (int) FourGuardianSymbol.WildHorizontal;
        ...
        {
            return CheckWild(2, 0, (int) FourGuardianSymbol.WildSquare, wheel);
        }

        private static bool CheckSquareMidle(WheelViewModel wheel)
        {
            return CheckWild(2, 1, (int) FourGuardianSymbol.WildSquare, wheel);
        }

        private static bool CheckSquareBottom(WheelViewModel wheel)
        {
            return CheckWild(2, 2, (int) FourGuardianSymbol.WildSquare, wheel);
        }
        ...
        private static void WildSqureExpanding(int position, WheelViewModel wheel)
        {
            ...
            for (var col = 1; col < 4; col++)
                for (var index = row; index < deep; index++)
                {
                    ...
                    if ((sym < 10) || (sym == (int) FourGuardianSymbol.WildSquare))
        ...
        {
            return CheckWild(1, 1, (int) FourGuardianSymbol.Wild, wheel);
        }

        private static bool CheckDiagonalRight(WheelViewModel wheel)
        {
            return CheckWild(3, 1, (int) FourGuardianSymbol.Wild, wheel);
        }

        private static bool CheckDiagonalMidle(WheelViewModel wheel)
        {
            return CheckWild(2, 1, (int) FourGuardianSymbol.Wild, wheel);
        }
